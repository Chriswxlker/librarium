document.addEventListener('DOMContentLoaded', function() {
    ...
    const isInactive = !!document.getElementById('publishers-inactive-table');
    const tbody = document.getElementById(isInactive ? 'publishers-inactive-tbody' : 'publishers-tbody');
    let paginationDiv = document.getElementById('pagination');
    let currentPage = window.pageData?.page || 1;
    let totalPages = window.pageData?.totalPages || 1;

    if (!tbody) return; // Si no hay tabla, no hacer nada
                ...
                <tr class="border-b border-gray-700 hover:bg-gray-700/30 transition-colors">
                    ...
                    <td class="py-4 px-4">
                        <div class="flex justify-end gap-2">
                            <form action="/publishers/activate/${publisher.id_publisher}" method="POST" class="inline-block">
                                <button 
                                    type="submit" 
    ...
    function fetchPublishers(page = 1) {
        let url = isInactive ? `/publishers/inactive?page=${page}` : `/publishers?page=${page}`;
        fetch(url, { headers: { 'Accept': 'application/json' } })
            .then(res => res.json())
            .then(data => {
                renderRows(data.publishers);
